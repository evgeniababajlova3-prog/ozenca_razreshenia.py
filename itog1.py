...
# ИЗМЕНЕНИЕ 2: Добавлены правильные параметры разрешения
# По дальности: Fs_range = 720 МГц
Fs_range = 720e6  # Гц
c = 3e8  # скорость света, м/с
RANGE_RESOLUTION = c / (2 * Fs_range)  # метров на отсчет

# ИЗМЕНЕНИЕ 3: Правильная формула для азимутального разрешения (SAR)
# Параметры системы (примерные значения - нужно уточнить из данных)
wavelength = 0.03  # длина волны, м (X-диапазон, 10 ГГц)
v = 100  # скорость платформы, м/с
R = 50000  # дальность до цели, м
La = 2.0  # длина антенны в азимутальном направлении, м

# Формула для разрешения по азимуту в SAR
AZIMUTH_RESOLUTION = La / 2  # теоретическое разрешение для реальной апертуры
# Или для синтезированной апертуры (если известны параметры обработки):
# T_synth = 2.0  # время синтеза, с
# AZIMUTH_RESOLUTION = wavelength / (2 * v * T_synth) * R  # упрощенная формула
...
# ИЗМЕНЕНИЕ 4: Добавлена функция загрузки HDF5 вместо генерации РЛИ
...
def generate_sinc_signal_from_section(section, window_size):
    ...
    # ИЗМЕНЕНИЕ 5: ось от 0 до 128 вместо -64 до +64
    t = np.linspace(0, window_size, len(section))

    return t, section_db, section_norm


# ИЗМЕНЕНИЕ 6: Добавлена функция конвертации в метры
...
def main():
    # ИЗМЕНЕНИЕ 7: Замена генерации на загрузку из HDF5
...
СтерЖенёк(Ферритовый), [10.11.2025 1: 35]
...
plt.plot(target_data['h_t_interp'], target_data['h_sinc_interp'], 'g-', linewidth=1.5, label='Интерполированный сигнал',
         alpha=0.8)
...
plt.plot(target_data['v_t_interp'], target_data['v_sinc_interp'], 'g-', linewidth=1.5, label='Интерполированный сигнал',
         alpha=0.8)
...
for file in glob.glob(os.path.join(result_folder, "*")):
    if file not in pdf_files and os.path.isfile(file) and not file.endswith('.hdf5'):
        os.remove(file)
for target_dir in glob.glob(os.path.join(result_folder, "target_*")):
    import shutil
